...
GraphSKR <- function(
  ...
  GraphSKR <- ggpubr::ggarrange(
    ggplot2::ggplot()+
      ggplot2::stat_function(fun = \(x) intercept_ref_TADs + slope_ref_TADs*x^2,
                             linewidth = 1,
                             linetype = 2,
                             col = "black") +
      ggdensity::geom_hdr(data = MomentsDataFrame|>
    ...
    ggplot2::ggplot() +
      ...
      ggplot2::geom_smooth(data = MomentsDataFrame |>
                             dplyr::filter(Number > 0),
      ...
      ggpubr::stat_regline_equation(data = MomentsDataFrame |>
                                      dplyr::filter(Number == 0),
